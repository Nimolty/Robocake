...
def prepare_model_and_data(args, device, use_gpu, prior_model=None, residual_model=None):
    ...
    return residual_model, prior_model, eval_data_list, residual_eval_out_path

def inference_after_training(residual_model, prior_model, args, 
                             data_dict, residual_eval_out_path, use_gpu, device):
    emd_loss = EarthMoverLoss()
    chamfer_loss = ChamferLoss()
    h_loss = HausdorffLoss()
    ...
    with torch.no_grad():
        for step_id in tqdm(range(st_idx, ed_idx)):
            ...
            if args.residual_input_next_action == "GT":
                ...
                prior_pred_pos = torch.cat([prior_pred_pos_p, p_sample[step_id-1].unsqueeze(0)[:, n_particle:].to(device)], 1).unsqueeze(1)
...
def inference(residual_model, prior_model, this_eval_data, eval_data_class, 
              data_names, residual_eval_out_path, args, use_gpu, device):
    ...
    with torch.no_grad():
        for step_id in tqdm(range(st_idx, ed_idx)):
            ...
            if args.residual_input_next_action == "GT":
                ...
                prior_pred_pos = torch.cat([prior_pred_pos_p, torch.zeros_like(gt_pos[:, n_particle:])], 1).unsqueeze(1)
            elif args.residual_input_next_action == "LAST_GT":
                prior_pred_pos = torch.cat([prior_pred_pos_p, p_sample[step_id-1].unsqueeze(0)[:, n_particle:].to(device)], 1).unsqueeze(1)
...
def print_eval(args, residual_eval_out_path, loss_list_over_episodes):
...
if __name__ == "__main__":
    ...
    results = pool.starmap(inference, [(residual_model,prior_model, eval_data, args.eval_data_class, args.data_names, 
                                        residual_eval_out_path, args, use_gpu, device) 
    ...
    print_eval(args, residual_eval_out_path, loss_list_over_episodes)
