...
from models.prior_model_distributed import Prior_Model as Model
    ...
    def model_rollout(
        ...
        for i in range(act_seqs.shape[1]):
            ...
            for j in range(act_seqs.shape[2]):
                ...
                # pdb.set_trace()
                pred_pos, pred_motion_norm, std_cluster = self.model(inputs)
...
#     model = Model(args, use_gpu)
    ...
    try:
        model = Model(args, use_gpu)
        print("model_kp #params: %d" % count_parameters(model))
        checkpoint = load_checkpoint(args.resume_prior_path, device)
        model = load_single_model(model, checkpoint['model_state_dict'])
        model.eval()
