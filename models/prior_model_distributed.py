...
class DynamicsPredictor(nn.Module):
    ...
    # @profile
    def forward(self, inputs, stat, verbose=0, j=0, prior_remove_his_particles=3):
        args = self.args
        ...
        if prior_remove_his_particles:
            state_res_norm[:, :prior_remove_his_particles, :301, :] = 0
...
class Prior_Model(nn.Module):
    ...
    def forward(self, inputs, j=0,prior_remove_his_particles=3):
        """
        return:
        ret - predicted position of all particles, shape (n_particles, 3)
        """
        ret = self.dynamics_predictor(inputs, self.stat, self.args.verbose_model, j=j, prior_remove_his_particles=prior_remove_his_particles)
        return ret
