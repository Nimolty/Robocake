...
class Planner(object):
    def __init__(self, args, taichi_env, env_init_state, scene_params, n_particle, n_shape, prior_model, residual_model, all_p, 
    ...
    def model_rollout(
        ...
        for i in range(act_seqs.shape[1]):
            ...
            for j in range(act_seqs.shape[2]):
                ...
                # pdb.set_trace()
                prior_pred_pos_p, _, _ = self.prior_model(inputs, prior_remove_his_particles=self.args.prior_remove_his_particles)
                if self.args.residual_input_next_action == "ZERO":
                    prior_pred_pos = torch.cat([prior_pred_pos_p, torch.zeros_like(state_cur[:, -1, self.n_particle:]).float()], 1).unsqueeze(1)
...
def main():
    ...
    print("prior model_kp #params: %d" % count_parameters(prior_model))
    ...
    residual_model = Residual_Model(args, use_gpu)
    print("residual model_kp #params: %d" % count_parameters(residual_model))
    ...
    # state is a dict, keys : "state", "softness", "is_copy"
    # all_p : a list, length 118
    # goal_shapes : 
    planner = Planner(args=args, taichi_env=env, env_init_state=state, scene_params=scene_params, n_particle=n_particle, 
                    n_shape=n_shape, prior_model=prior_model, residual_model=residual_model, all_p=all_p, goal_shapes=goal_shapes, task_params=task_params, use_gpu=use_gpu, 
