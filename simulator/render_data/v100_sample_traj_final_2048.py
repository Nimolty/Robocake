...
# from utils.utils import exists_or_mkdir
...
def store_physics_params(physics_params, path):
    with open(f"{path}/physics_params.npy", 'wb') as f:
...
def customize_normals_consistent(pcd, K=100):
    points = np.asarray(pcd.points)
    normals = np.asarray(pcd.normals)
    ...
    for i, ind in enumerate(indices):
        avg_normal = np.mean(normals[ind], axis=0)
...
# def adjust_normals(normals, k=6):
#     # 再次使用 K 最近邻
#     neigh = NearestNeighbors(n_neighbors=k)
#     neigh.fit(points)
#     indices = neigh.kneighbors(points, return_distance=False)
...
def poisson_mesh_reconstruct(rest, cube, prim_pos, visualize=False):
    ...
    grippers = rest.select_down_sample(gripper_label[0])

    labels = np.array(grippers.cluster_dbscan(eps=0.03, min_points=100))
    gripper1 = grippers.select_down_sample(np.where(labels == 0)[0])
    gripper2 = grippers.select_down_sample(np.where(labels > 0)[0])
    for gripper in [gripper1, gripper2]:
        ...
        customize_normals_consistent(gripper)
    ...
    customize_normals_consistent(cube)
...
def mesh_reconstruct(pcd, algo="filter", alpha=0.5, depth=8, visualize=False):
    if algo == "filter":
        ...
        customize_normals_consistent(pcd)
...
def patch(cube, prev_pcd, grippers, n_points, back, looking_ahead=5, surface=True, visualize=False):
    ...
    if not back:
        ...
        selected_pcd = selected_pcd.select_down_sample(inlier_ind)
...
def process_raw_pcd(pcd_all, visualize=False):
    ...
    # cube + gripper
    rest = pcd_all.select_down_sample(inliers, invert=True)
    ...
    cube = rest.select_down_sample(cube_label[0])

    cl, inlier_ind = cube.remove_statistical_outlier(nb_neighbors=40, std_ratio=1.5)
    cube = cube.select_down_sample(inlier_ind)
...
def main(args):
    ...
    prepare_mat_dir_list = sorted(glob.glob(os.path.join(prepare_pcd_dir, '*')))
    for i_idx in range(0, len(prepare_mat_dir_list)):
        prepare_mat_dir = prepare_mat_dir_list[i_idx]
        mat_idx = int(prepare_mat_dir.split('/')[-1])


        prepare_pcd_dir_list = sorted(glob.glob(os.path.join(prepare_mat_dir, '*')))
        ...
        # for vid_idx in range(42, 43):
            ...
            print(f'========== Mat {mat_idx} Video {data_idx} ==========')
            rollout_path = os.path.join(output_path, f"{mat_idx:04d}", f"{data_idx:03d}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--prepare_pcd_dir", default="/nvme/tianyang/prepare_pybert_data_2048/sample_output_2048")
    parser.add_argument("--output_path", default="/nvme/tianyang/sample_pybert_data_2048")
